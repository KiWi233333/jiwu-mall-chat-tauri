// tray.rs
use tauri::{
  menu::{MenuBuilder, MenuItemBuilder}, tray::{MouseButton, MouseButtonState, TrayIconBuilder, TrayIconEvent}, AppHandle, Emitter, Manager
};
  ...
  TrayIconBuilder::new()
      ...
      .on_menu_event(move |app, event| match event.id().as_ref() {
          "setting" => {
              ...
              window.emit("router", Payload { message: "/setting".into() }).unwrap();
          }
          "to_host" => {
              let window = app.get_webview_window("main").unwrap();
              window.emit("open_url", Payload { message: "https://jiwu.kiwi2333.top".into() }).unwrap();
          }
          "quit" => {
              std::process::exit(0);
          }
