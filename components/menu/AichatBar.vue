...
<template>
  <ClientOnly>
    ...
    >
      <template #reference>
        <div
          class="h-3rem w-3rem flex-row-c-c rounded-1/2 bg-[var(--el-color-primary)] shadow-[#5d33f9] shadow-md shadow-opacity-60"
        >
          <i class="i-solar:ghost-bold bg-light p-0.8em" />
        </div>
      </template>
      <template #default>
        <section v-if="user.isLogin" class="w-90vw md:w-450px">
          <!-- header -->
