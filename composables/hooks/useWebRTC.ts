...
export function useWebRTC(openDialog: (type: CallTypeEnum, { confirmCall, rejectCall }: { confirmCall: () => void, rejectCall: () => void }) => void) {
  ...
  const isWaitingAnswer = ref(false); // 是否等待对方回应
  // 流相关状态
   ...
   */
  const endCall = async (status: CallStatusEnum | undefined = CallStatusEnum.END) => {
    try {
  ...
  async function handleAnswer({ data: answer, roomId }: WSRtcCallMsg) {
    try {
      if (peerConnection.value) {
        ...
        // 3. 通知服务器通话已建立
        if (isSender.value) { // 发起者 发送 ANSWER 信令
          if (!roomId) {
            ElMessage.error("房间号不存在，请重新连接！");
            endCall();
            return;
          ...
          // 5. 发起者 -  添加ICE候选者处理
          peerConnection.value.onicecandidate = async (event) => {
            if (event.candidate && rtcMsg.value.roomId) {
              try {
                // console.log(user.userInfo.username, "收到 ICE candidate 信令:", event.candidate);
    ...
    try {
      ...
      if (localStream.value) {
        ...
        if (oldAudioTrack) {
          ...
          localStream.value.addTrack(newAudioTrack || oldAudioTrack);
        }
      }
    }
    catch (error) {
  ...
  //   volumeLevel.value = volumeLevel.value === 1 ? 0 : 1; // 切换音量
  //   // 切换对方音量
  //   if (peerConnection.value) {
  //     const audioTrack = peerConnection.value.getSenders().find(sender => sender.track && sender.track.kind === "");
